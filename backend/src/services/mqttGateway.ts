...
import { type Tank, type TankStatus, type TankContents } from '../domain/models'
...
console.log('[MQTT Gateway] Configuration:', {
  ...
  url: env.mqtt.url,
  mode: env.mqtt.enableMock || !env.mqtt.url ? 'mock' : 'live',
})
...
const TANK_TEMP_TOPIC = (ix: number) => `cuve/${ix}/temp`
const TANK_SETPOINT_TOPIC = (ix: number) => `cuve/${ix}/consigne`
const TANK_STATE_TOPIC = (ix: number) => `cuve/${ix}/etat`
const TANK_CONTENTS_TOPIC = (ix: number) => `cuve/${ix}/contenu`
const TANK_SET_SETPOINT_TOPIC = (ix: number) => `cuve/${ix}/set/consigne`
const TANK_SET_CONTENTS_TOPIC = (ix: number) => `cuve/${ix}/set/contenu`
const TANK_SET_MODE_TOPIC = (ix: number) => `cuve/${ix}/set/mode`

// Map pour suivre la dernière mise à jour de chaque cuve (pour détecter offline)
const lastTankUpdate = new Map<number, number>()
const OFFLINE_TIMEOUT_MS = 60_000 // 1 minute sans message = offline
let offlineCheckInterval: NodeJS.Timeout | undefined
...
const syncConfig = (cuveries: CuverieConfig[]) => {
  ...
  cuveries.forEach((cuverie) => {
    ...
    cuverie.tanks.forEach((tank) => {
      const created = tankRepository.upsertFromConfig(cuverie.id, tank)
  ...
  const finalConfig = configRepository.list()
    ...
    try {
      switch (dataType) {
        ...
        case 'etat':
          ...
          const statusMap: Record<string, TankStatus> = {
            'FROID': 'cooling',
            'CHAUD': 'heating',
            'ATT': 'idle',
            'ARRET': 'idle',
        ...
        case 'contenu':
          ...
          if (affectation.length > 0) {
            ...
            if (currentTank) {
              ...
              tankRepository.updateContents(tankIx, updatedContents)
              ...
              const newContents: TankContents = {
                grape: affectation,
                vintage: new Date().getFullYear(),
                volumeLiters: 0,
              }
              tankRepository.updateContents(tankIx, newContents)
...
export const mqttGateway = {
  start: () => {
    ...
    const initialConfig = configRepository.list()
  ...
  publishCommand: (tankIx: number, command: Record<string, unknown>) => {
    ...
    switch (commandType) {
      ...
      case 'contents':
        ...
        const contentsValue = command.value as TankContents | string | undefined
      ...
      default:
        ...
        topic = `${env.mqtt.topics.commands}/${tankIx}`
